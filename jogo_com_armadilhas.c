...
void reiniciarJogo(int *x, int *y, INIMIGO inimigo[], int *vidas, int mapa[MAPA_LARGURA][MAPA_ALTURA])
{
    *x = ((rand() % 40) * LADO);
    *y = ((rand() % 40) * LADO);
    *vidas = 3;
...
for (int i = 0; i < N_TRAPS; i++)
{
  armadilhas[i][0] = (rand() % MAPA_LARGURA); // Coordenada X da armadilha
    ...
    mapa[armadilhas[i][0]][armadilhas[i][1]] = 2; // Marque a célula do mapa como uma armadilha (ou outro valor de sua escolha)
}
...
// Verifica colisão com inimigos
        ...
        {
            inimigo[i].xi, inimigo[i].yi, LADO, LADO
            }))
            {
                vidas--;
            }
            if (vidas <= 0)
            {
                reiniciarJogo(&x, &y, inimigo, &vidas, mapa);
            }
        }
    ...
    for (int i = 0; i < N_TRAPS; i++)
{
    ...
    for (int x = 0; x < MAPA_LARGURA; x++)
    {
        if (mapa[x][y] == 2)
        {
            Rectangle trapRect = {x * LADO, y * LADO, LADO, LADO};

            if (CheckCollisionRecs((Rectangle){x * LADO, y * LADO, LADO, LADO}, (Rectangle){x, y, LADO, LADO}) ||
                CheckCollisionRecs(trapRect, (Rectangle){x * LADO, y * LADO, LADO, LADO}))
            {
                vidas--;
            }
            if (vidas <= 0)
