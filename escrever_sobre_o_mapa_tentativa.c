...
{
    FILE *mapa_arq = fopen(nome_arquivo, "a"); // Abra o arquivo no modo de anexação
    ...
    for (int i = 0; i < num_inimigos; i++)
    {
        ...
        {
            // Escreva 'J' na posição do inimigo no arquivo
            fprintf(mapa_arq, "J");
        }

        // Adicione um espaço ou caractere de nova linha, conforme necessário
        if (i < num_inimigos - 1)
        {
            fprintf(mapa_arq, " "); // Adicione um espaço entre inimigos
        }
        else
        {
            fprintf(mapa_arq, "\n"); // Adicione uma nova linha após o último inimigo
        }
    }
...
void atualizarMapaGrafico(char mapa[][MAPA_COLUNA])
{
    for (int i = 0; i < MAPA_LINHA; i++)
    {
        for (int j = 0; j < MAPA_COLUNA; j++)
        {
            ...
            drawSquare(j, i, LADO, color); // Atualize a representação gráfica
        }
    }
}
...
if (leMapa("maps.txt", &jogador, inimigos, &num_inimigos, mapa)) {
    atualizarMapaGrafico(mapa); // Atualiza a representação gráfica inicial
}
